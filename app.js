...
app.get("/", function(req, res){
  ...
  Blog.find({}, (err, posts) => {
    if(!err){
      res.render("home", {
        startingContent: homeStartingContent.postContent,
        posts: posts
      });
    }
  });
...
app.get("/posts/:postName", function(req, res){
  ...
  Blog.findById(requestedPost, (err, foundPost) => {
    if(!err){
      if(foundPost){
        res.render("post", {title: foundPost.title, content: foundPost.postContent})
      }else{
        res.render("post", {title: "Cannot found this post!", content: "Try again!"});
      }
    }else{
      console.log(err);
    }
